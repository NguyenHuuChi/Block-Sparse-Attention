...
import block_sparse_attn_fused_pos_cuda
import block_sparse_attn_fused_pos_local_cuda
...
def prepare_varlen_inputs(
    q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, seqlen: int
...
def block_sparse_attn_pos_fused(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    base_blockmask: Optional[torch.Tensor],
    head_mask_type: Optional[torch.Tensor] = None,
    rel_h_flat: Optional[torch.Tensor] = None,
    rel_w_flat: Optional[torch.Tensor] = None,
    H: Optional[int] = None,
    W: Optional[int] = None,
    softmax_scale: Optional[float] = None,
) -> torch.Tensor:
    """Block-sparse attention with rel_h/rel_w fused inside the CUDA kernel."""
    ...
    if base_blockmask is not None:
        ...
        if row_blockmask.dim() == 4 and row_blockmask.size(0) == batch_size * num_heads:
            row_blockmask = row_blockmask.reshape(
                batch_size, num_heads, row_blockmask.size(-2), row_blockmask.size(-1)
            )
        row_blockmask = maybe_contiguous(row_blockmask)

    q_unpad, k_unpad, v_unpad = [
        maybe_contiguous(x) for x in (q_unpad, k_unpad, v_unpad)
    ]
    rel_h_flat = maybe_contiguous(rel_h_flat)
    # if rel_h_flat is not None:
    #     print(
    #         f"[PYDEBUG] rel_h_flat.shape={rel_h_flat.shape}, stride={rel_h_flat.stride()}"
    #     )
    rel_w_flat = maybe_contiguous(rel_w_flat)
    # if start_index_block_tile_rel_w is not None and not torch.is_tensor(
    #     start_index_block_tile_rel_w
    # ):
    #     start_index_block_tile_rel_w = torch.tensor(
    #         start_index_block_tile_rel_w, device=q.device, dtype=torch.int32
    #     )
    # if torch.is_tensor(start_index_block_tile_rel_w):
    #     start_index_block_tile_rel_w = maybe_contiguous(start_index_block_tile_rel_w)

    output, _ = block_sparse_attn_fused_pos_cuda.fwd(
        q_unpad,
        k_unpad,
        v_unpad,
        cu_seqlens,
    ...
    return output.reshape(batch_size, seqlen, num_heads, head_dim)


def block_sparse_attn_pos_fused_local(
    q: torch.Tensor,
    k: torch.Tensor,
    v: torch.Tensor,
    base_blockmask: Optional[torch.Tensor],
    ...
    """Local-only block-sparse attention with compact rel_h/rel_w fused in the local kernel."""
    ...
    output, _ = block_sparse_attn_fused_pos_local_cuda.fwd(
        q_unpad,
        k_unpad,
        v_unpad,
        cu_seqlens,
...
def block_sparse_attn(
    ...
    head_mask_type_modified, blocksparse_head_num = replace_ones_with_count(
        head_mask_type.clone()
    )
