...
template<typename TensorGRel, typename TensorSRel, typename Element>
inline __device__ void load_rel_compact_to_smem(TensorGRel const &gRel, TensorSRel &sRel, int tidx, int valid_rows, int valid_cols) {
    for (int idx = tidx; idx < size(sRel); idx += blockDim.x) {
        const int row = idx % size<0>(sRel);
        const int col = idx / size<0>(sRel);
        sRel(row, col) = (row < valid_rows && col < valid_cols) ? gRel(row, col) : Element(0);
    }
}
...
inline __device__ void compute_block_attn_sam(
    ...
    auto sRelBase = sVt.data() + size(sVt);
    Tensor sRel_h = make_tensor(sRelBase, make_layout(Shape<Int<kBlockM>, Int<14>>{}, Stride<_1, Int<kBlockM>>{}));
    Tensor sRel_w = make_tensor(sRel_h.data() + size(sRel_h), make_layout(Shape<Int<kBlockM>, Int<14>>{}, Stride<_1, Int<kBlockM>>{}));
    ...
    // Width-16 padded cooperative rel-pos copy experiment kept for reference.
    // auto [gRel_h, gRel_w] = load_rel_h_rel_w<Kernel_traits, Is_even_MN, Is_even_K>(params, bidb, bidh, m_block, 0);
    // typename Kernel_traits::GmemTiledCopyRelH gmem_tiled_copy_Rel;
    // auto gmem_thr_copy_Rel = gmem_tiled_copy_Rel.get_thread_slice(tidx);
    // Tensor tRel_hgRel_h = gmem_thr_copy_Rel.partition_S(gRel_h);
    // Tensor tRel_hsRel_h = gmem_thr_copy_Rel.partition_D(sRel_h);
    // Tensor tRel_wgRel_w = gmem_thr_copy_Rel.partition_S(gRel_w);
    // Tensor tRel_wsRel_w = gmem_thr_copy_Rel.partition_D(sRel_w);
    ...
    // Full-tile pos-style local bias path kept for reference.
    ...
    // Tensor cRel_h = make_identity_tensor(make_shape(size<0>(gRel_h), size<1>(gRel_h)));
    ...
    // === PROLOGUE: Load Q and compact rel_h/rel_w===
    ...
    if (use_rel_pos) {
        // Width-16 padded cooperative rel-pos copy experiment kept for reference.
        // FLASH_NAMESPACE::copy<Is_even_MN, /*Is_even_K=*/false>(
        //     gmem_tiled_copy_Rel, tRel_hgRel_h, tRel_hsRel_h, tRel_h_cRel_h, tRel_hpRel_h,
        //     binfo.actual_seqlen_q - m_block * kBlockM
        // );
        // FLASH_NAMESPACE::copy<Is_even_MN, /*Is_even_K=*/false>(
        //     gmem_tiled_copy_Rel, tRel_wgRel_w, tRel_wsRel_w, tRel_w_cRel_w, tRel_wpRel_w,
        //     binfo.actual_seqlen_q - m_block * kBlockM
        // );
        // cute::cp_async_fence();
        auto [gRelHCompact, gRelWCompact] = load_rel_h_rel_w<Kernel_traits, Is_even_MN, Is_even_K>(params, bidb, bidh, m_block, 0);
        load_rel_compact_to_smem<decltype(gRelHCompact), decltype(sRel_h), Element>(gRelHCompact, sRel_h, tidx, binfo.actual_seqlen_q - m_block * kBlockM, params.k_h);
        load_rel_compact_to_smem<decltype(gRelWCompact), decltype(sRel_w), Element>(gRelWCompact, sRel_w, tidx, binfo.actual_seqlen_q - m_block * kBlockM, params.k_w);
    }
    ...
    for (int masking_step = 0; masking_step < n_masking_steps; ++masking_step, --n_block) {
        ...
        if (is_skip) {
            ...
            if (use_rel_pos) {
                ...
                add_rel_w_from_gmem_fragment<Kernel_traits, ElementAccum>(acc_s, tScS, sRel_w, params, n_block, binfo.actual_seqlen_k);
            }
