 ...
 ******************************************************************************/
/******************************************************************************
 * Adapted by Hoai-Chau Tran and Huu-Chi Nguyen from https://github.com/Dao-AILab/flash-attention
 ******************************************************************************/
/******************************************************************************
...
template<typename Kernel_traits, typename TensorGRelH, typename TensorSRelH>
inline __device__ void load_rel_h_to_smem(TensorGRelH const &gRel_h, TensorSRelH &sRel_h, int tidx, int valid_rows) {
    using Element = typename Kernel_traits::Element;
    for (int idx = tidx; idx < Kernel_traits::kBlockM * Kernel_traits::kRelHLastDim; idx += Kernel_traits::kNThreads) {
        const int row = idx / Kernel_traits::kRelHLastDim;
        const int col = idx % Kernel_traits::kRelHLastDim;
        sRel_h(row, col) = row < valid_rows ? gRel_h(row, col) : Element(0);
    }
}


template<typename Kernel_traits, typename TensorGRelW, typename TensorSRelW>
inline __device__ void load_rel_w_to_smem(TensorGRelW const &gRel_w, TensorSRelW &sRel_w, int tidx, int valid_rows, int rel_w_last_dim) {
    using Element = typename Kernel_traits::Element;
    for (int idx = tidx; idx < Kernel_traits::kBlockM * rel_w_last_dim; idx += Kernel_traits::kNThreads) {
        const int row = idx / rel_w_last_dim;
        const int col = idx % rel_w_last_dim;
        sRel_w(row, col) = row < valid_rows ? gRel_w(row, col) : Element(0);
    }
}


template<typename Kernel_traits, typename ThrMma, typename TensorSRelW>
inline __device__ auto make_rel_w_window_partition(ThrMma const &thr_mma, TensorSRelW &sRel_w, int start_index_rel_w) {
    auto sRel_w_window = make_tensor(
        sRel_w.data() + start_index_rel_w * Kernel_traits::kBlockM,
        make_layout(
            Shape<Int<Kernel_traits::kBlockM>, Int<Kernel_traits::kBlockN>>{},
            Stride<_1, Int<Kernel_traits::kBlockM>>{}
        )
    );
    return thr_mma.partition_C(sRel_w_window);
}

template<typename Kernel_traits, typename ElementAccum, typename TensorAccS, typename TensorCoordS, typename TensorSRelW>
inline __device__ void add_rel_w_from_smem_fragment(
    TensorAccS &acc_s,
    TensorCoordS const &tScS,
    TensorSRelW const &sRel_w,
    int start_index_rel_w
) {
    CUTE_STATIC_ASSERT_V(size<0>(acc_s) == size<0>(tScS));
    CUTE_STATIC_ASSERT_V(size<1>(acc_s) == size<1>(tScS));
    CUTE_STATIC_ASSERT_V(size<2>(acc_s) == size<2>(tScS));
    #pragma unroll
    for (int mi = 0; mi < size<0>(acc_s); ++mi) {
        ...
        for (int ni = 0; ni < size<1>(acc_s); ++ni) {
            ...
            for (int ki = 0; ki < size<2>(acc_s); ++ki) {
                auto coord = tScS(mi, ni, ki);
                const int row = get<0>(coord);
                const int col = get<1>(coord);
                acc_s(mi, ni, ki) = static_cast<ElementAccum>(
                    acc_s(mi, ni, ki) + static_cast<ElementAccum>(sRel_w(row, start_index_rel_w + col))
                );
            }
        }
    }
...
// template<typename Kernel_traits, typename ElementAccum, typename TensorScores, typename TensorSRelH, typename Params>
// inline __device__ void add_rel_h_from_smem(TensorScores &scores, TensorSRelH const &sRel_h, Params const &params, int n_block, int actual_seqlen_k) {
//     #pragma unroll
//     for (int mi = 0; mi < size<0>(scores); ++mi) {
//         #pragma unroll
//         for (int ni = 0; ni < size<1>(scores); ++ni) {
//             const int global_k = n_block * Kernel_traits::kBlockN + ni;
//             const int kh = global_k / params.k_w;
//             if (global_k < actual_seqlen_k && kh < params.k_h) {
//                 scores(mi, ni) = static_cast<ElementAccum>(scores(mi, ni) + static_cast<ElementAccum>(sRel_h(mi, kh)));
//             }
//         }
//     }
// }
...
template<typename Kernel_traits, typename ElementAccum, typename TensorAccS, typename TensorCoordS, typename TensorGRelW, typename Params>
inline __device__ void add_rel_w_from_gmem_fragment(TensorAccS &acc_s, TensorCoordS const &tScS, TensorGRelW const &gRel_w, Params const &params, int n_block, int actual_seqlen_k) {
    CUTE_STATIC_ASSERT_V(size<0>(acc_s) == size<0>(tScS));
    CUTE_STATIC_ASSERT_V(size<1>(acc_s) == size<1>(tScS));
    CUTE_STATIC_ASSERT_V(size<2>(acc_s) == size<2>(tScS));
    #pragma unroll
    for (int mi = 0; mi < size<0>(acc_s); ++mi) {
        ...
        for (int ni = 0; ni < size<1>(acc_s); ++ni) {
            ...
            for (int ki = 0; ki < size<2>(acc_s); ++ki) {
                ...
                const int row = get<0>(coord);
                const int col = get<1>(coord);
                const int global_k = n_block * Kernel_traits::kBlockN + col;
                if (global_k < actual_seqlen_k) {
                    const int kw = global_k % params.k_w;
                    acc_s(mi, ni, ki) = static_cast<ElementAccum>(acc_s(mi, ni, ki) + static_cast<ElementAccum>(gRel_w(row, kw)));
                }
            }
        }
    }
...
template<typename Kernel_traits, typename ElementAccum, typename TensorAccS, typename TensorCoordS, typename TensorSRelH, typename Params>
inline __device__ void add_rel_h_from_smem_fragment(TensorAccS &acc_s, TensorCoordS const &tScS, TensorSRelH const &sRel_h, Params const &params, int n_block, int actual_seqlen_k) {
    CUTE_STATIC_ASSERT_V(size<0>(acc_s) == size<0>(tScS));
    CUTE_STATIC_ASSERT_V(size<1>(acc_s) == size<1>(tScS));
    CUTE_STATIC_ASSERT_V(size<2>(acc_s) == size<2>(tScS));
    #pragma unroll
        ...
        for (int ni = 0; ni < size<1>(acc_s); ++ni) {
            ...
            for (int ki = 0; ki < size<2>(acc_s); ++ki) {
                ...
                if (global_k < actual_seqlen_k) {
                    const int kh = global_k / params.k_w;
                    if (kh < params.k_h) {
                        acc_s(mi, ni, ki) = static_cast<ElementAccum>(acc_s(mi, ni, ki) + static_cast<ElementAccum>(sRel_h(row, kh)));
                    }
                }
            }
        }
...
inline __device__ auto load_rel_h_rel_w(
    ...
    // const index_t row_offset_rel_h = params.rel_h_batch_stride  * bidb
    //     + bidh * params.rel_h_head_stride
    //     + m_block * kBlockM * params.rel_h_row_stride
    //     + n_block * kBlockN * params.rel_h_col_stride;

    const index_t row_offset_rel_w = params.rel_w_batch_stride  * bidb
    ...
    Tensor gRelW = make_tensor(make_gmem_ptr(reinterpret_cast<Element *>(params.rel_w_ptr) + row_offset_rel_w),
                              Shape<Int<kBlockM>, Int<kBlockN>>{},
                              make_stride(params.rel_w_row_stride, params.rel_w_col_stride));
...
inline __device__ void compute_block_attn_sam(
    ...
    Tensor gRel_h = make_tensor(make_gmem_ptr(reinterpret_cast<Element *>(params.rel_h_ptr) + row_offset_rel_h),
                            Shape<Int<kBlockM>, Int<Kernel_traits::kRelHLastDim>>{},
                            // make_stride(_1{}, params.rel_h_col_stride));
                            make_stride(params.rel_h_row_stride, _1{}));
    Tensor gRel_w = make_tensor(make_gmem_ptr(reinterpret_cast<Element *>(params.rel_w_ptr) + row_offset_rel_w),
                            Shape<Int<kBlockM>, Int<Kernel_traits::kRelWLastDim>>{},
                            make_stride(params.rel_w_row_stride, _1{}));
    ...
    Tensor sRel_h = make_tensor(sVt.data() + size(sVt), typename Kernel_traits::SmemLayoutRelH{}); // [kBlockM, rel_h_last_dim] in smem
    Tensor sRel_w = make_tensor(sRel_h.data() + size(sRel_h), typename Kernel_traits::SmemLayoutRelW{}); // [kBlockM, rel_w_last_dim] in smem
    ...
    // Experimental rel-pos copies used a separate tiled-copy path.
    // typename Kernel_traits::GmemTiledCopyRelH gmem_tiled_copy_RelH;
    // auto gmem_thr_copy_RelH = gmem_tiled_copy_RelH.get_thread_slice(tidx);
    // Tensor tRel_hgRel_h = gmem_thr_copy_RelH.partition_S(gRel_h);
    // Tensor tRel_hsRel_h = gmem_thr_copy_RelH.partition_D(sRel_h);
    // Tensor tRel_wgRel_w = gmem_thr_copy_RelH.partition_S(gRel_w);
    // Tensor tRel_wsRel_w = gmem_thr_copy_RelH.partition_D(sRel_w);
    Tensor tRel_hgRel_h = gmem_thr_copy_QKV.partition_S(gRel_h);
    Tensor tRel_hsRel_h = gmem_thr_copy_QKV.partition_D(sRel_h);
    Tensor tRel_wgRel_w = gmem_thr_copy_QKV.partition_S(gRel_w);
    Tensor tRel_wsRel_w = gmem_thr_copy_QKV.partition_D(sRel_w);
    ...
    auto c_tensor_o = make_tensor(
        make_rmem_ptr<ElementAccum>(&c_dummy_o),
        make_layout(Shape<Int<kBlockM>, Int<kHeadDim>>{})
    );
    Tensor acc_o = thr_mma.partition_fragment_C(c_tensor_o);

    ElementAccum c_dummy_s;
    auto c_tensor_s = make_tensor(
        make_rmem_ptr<ElementAccum>(&c_dummy_s),
    ...
    // rel_w now stays compact as [kBlockM, k_w] and is reconstructed exactly in-kernel.
    Tensor tsgRelW = thr_mma.partition_C(sRel_w);
    ...
    Tensor cRel_h = make_identity_tensor(make_shape(size<0>(sRel_h), size<1>(sRel_h)));
    Tensor cRel_w = make_identity_tensor(make_shape(size<0>(sRel_w), size<1>(sRel_w)));
    ...
    // Tensor tRel_h_cRel_h = gmem_thr_copy_RelH.partition_S(cRel_h);
    // Tensor tRel_w_cRel_w = gmem_thr_copy_RelH.partition_S(cRel_w);
    Tensor tRel_h_cRel_h = gmem_thr_copy_QKV.partition_S(cRel_h);
    Tensor tRel_w_cRel_w = gmem_thr_copy_QKV.partition_S(cRel_w);

    Tensor tQpQ = make_tensor<bool>(make_shape(size<2>(tQsQ)));
    Tensor tKVpKV = make_tensor<bool>(make_shape(size<2>(tKsK)));
    Tensor tRel_hpRel_h = make_tensor<bool>(make_shape(size<2>(tRel_hsRel_h)));
    Tensor tRel_wpRel_w = make_tensor<bool>(make_shape(size<2>(tRel_wsRel_w)));
    ...
    #pragma unroll
    for (int k = 0; k < size(tRel_hpRel_h); ++k) { tRel_hpRel_h(k) = get<1>(tRel_h_cRel_h(0, 0, k)) < params.rel_h_last_dim; }
    #pragma unroll
    for (int k = 0; k < size(tRel_wpRel_w); ++k) { tRel_wpRel_w(k) = get<1>(tRel_w_cRel_w(0, 0, k)) < params.rel_w_last_dim; }

    // === PROLOGUE: Load Q and Rel_h===
    ...
    if (use_rel_pos) {
        ...
        FLASH_NAMESPACE::copy<Is_even_MN, Is_even_K>(
            gmem_tiled_copy_QKV, tRel_wgRel_w, tRel_wsRel_w, tRel_w_cRel_w, tRel_wpRel_w,
            binfo.actual_seqlen_q - m_block * kBlockM
        );
        cute::cp_async_fence();
            ...
            // );
            // FLASH_NAMESPACE::copy<Is_even_MN, /*Is_even_K=*/false, /*Clear_OOB_MN=*/true>(
            //     gmem_tiled_copy_RelH, tRel_wgRel_w, tRel_wsRel_w, tRel_w_cRel_w, tRel_wpRel_w,
            //     binfo.actual_seqlen_q - m_block * kBlockM
            // );
            // load_rel_w_to_smem<Kernel_traits>(gRel_w, sRel_w, tidx, binfo.actual_seqlen_q - m_block * kBlockM, params.rel_w_last_dim);
    ...
    //         )
    //     );
    // auto tssRelW = thr_mma.partition_C(sRel_w_window);

    // auto sRel_h_window = make_tensor(
    //         sRel_h.data(),
    //         make_layout(
    //             Shape<Int<Kernel_traits::kBlockM>, Int<Kernel_traits::kBlockN>>{},
    //             Stride<_1, Int<Kernel_traits::kBlockM>>{}
    ...
    int leap = 0;
    // int start_index_rel_h = params.start_index_block_tile_rel_h + mask_val; // take the starting index value of mask_val th block
    // const int *start_index_block_tile_rel_h = reinterpret_cast<const int *>(params.start_index_block_tile_rel_h);
    // const int *start_index_block_tile_rel_w = reinterpret_cast<const int *>(params.start_index_block_tile_rel_w);
    // (void)start_index_block_tile_rel_w;
    ...
    for (int masking_step = 0; masking_step < n_masking_steps; ++masking_step, --n_block) {
        ...
        if (is_skip) {
            ...
            if (use_rel_pos) {
                // const int start_index_rel_h = start_index_block_tile_rel_h != nullptr ? start_index_block_tile_rel_h[n_block] : 0;
                // auto tssRelH = make_rel_h_window_partition<Kernel_traits>(thr_mma, sRel_h, start_index_rel_h);
                // axpby(1.0, tsgRelW,1.0 ,acc_s);
                // add_rel_w_from_gmem_fragment<Kernel_traits, ElementAccum>(acc_s, tScS, gRel_w, params, n_block, binfo.actual_seqlen_k);
            ...
            if (use_rel_pos) {
                add_rel_h_from_smem_fragment<Kernel_traits, ElementAccum>(acc_s, tScS, sRel_h, params, n_block, binfo.actual_seqlen_k);
                // DEBUG: rel_h restored to safe scalar load/add path.
            }
    ...
    for (; !is_last_block && n_block >= n_block_min; n_block = next_block_col_idx) {
        ...
        // start_index_rel_h = *params.start_index_block_tile_rel_h + mask_val;
...
template<typename Kernel_traits, bool Is_even_MN, bool Is_even_K, typename Params>
inline __device__ void compute_block_pos_sam(
    const Params &params, const int bidb, const int bidh, const int m_block
) {
    using Element = typename Kernel_traits::Element;
    using ElementAccum = typename Kernel_traits::ElementAccum;
    ...
    for (int masking_step = 0; masking_step < n_masking_steps; ++masking_step, --n_block) {
        ...
        if (is_skip) {
            ...
            if (n_block > n_block_min && !is_last_block) {
                tKgK.data() = tKgK.data() + (-index_t(kBlockN * leap * params.k_row_stride));
                tsgRelH.data() = tsgRelH.data() + (-index_t(kBlockN * leap * params.rel_h_col_stride));
                // tsgRelW.data() = tsgRelW.data() + (-index_t(kBlockN * leap * params.rel_w_col_stride));
                // FLASH_NAMESPACE::copy</*Is_even_MN=*/true, Is_even_K>(gmem_tiled_copy_QKV, tKgK, tKsK, tKVcKV, tKVpKV);
                cute::cp_async_fence();
            }
